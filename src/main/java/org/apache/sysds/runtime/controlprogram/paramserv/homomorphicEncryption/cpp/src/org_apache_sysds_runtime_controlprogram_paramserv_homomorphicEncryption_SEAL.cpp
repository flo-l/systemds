#include "org_apache_sysds_runtime_controlprogram_paramserv_homomorphicEncryption_SEAL.h"
...
struct GlobalState {
    ...
    explicit GlobalState(double _scale) : context(generateParameters()), a(generate_a(context)), scale(_scale) {};
};
...
public:
    explicit Client(GlobalState global_state) : _gs(move(global_state)), _encoder(_gs.context) {
...
public:
    explicit Server(GlobalState global_state) : _gs(move(global_state)) {};
...
// TODO: serialization
// TODO: evaluate zstandard for compressed serialization
int main() {
    double scale = pow(2.0, 40);
    size_t num_clients = 3;
    GlobalState gs(scale);

    size_t slot_count = gs.context.first_context_data()->parms().poly_modulus_degree() >> 1;

    // 1. setup
    std::vector<Client> clients;
    clients.reserve(num_clients);
    for (size_t i = 0; i < num_clients; i++) {
        clients.emplace_back(gs);
    }
    ...
    partial_public_keys.reserve(num_clients);
    for (Client& client : clients) {
        partial_public_keys.push_back(client.partial_public_key().data());
    }
    server.accumulate_partial_public_keys(gsl::span(partial_public_keys));

    for (Client& client : clients) {
        client.set_public_key(server.public_key());
    }

    // 3. homomorphic sum
    vector<vector<double>> plaintext_data;
    plaintext_data.reserve(num_clients);

    vector<Ciphertext> client_data;
    client_data.reserve(num_clients);
    for (size_t i = 0; i < num_clients; i++) {
        plaintext_data.push_back(random_plaintext_data(slot_count * 2)); // * 2 because we can fit 2 doubles in 1 slot
        client_data.push_back(clients[i].encrypted_data(gsl::span(plaintext_data[i])));
    }

    Ciphertext sum_data = server.sum_data(std::move(client_data));

    // 4. partial decryptions
    vector<Plaintext> partial_decryptions;
    partial_decryptions.reserve(num_clients);
    for (size_t i = 0; i < num_clients; i++) {
        partial_decryptions.push_back(clients[i].partial_decryption(sum_data));
    }

    // 5. average
    vector<double> averages = server.average(sum_data, partial_decryptions);

    // check
    vector<double> expected;
    expected.reserve(averages.size());
    for (size_t i = 0; i < averages.size(); i++)
    {
        double sum = 0.0;
        for (vector<double>& v : plaintext_data) {
            sum += v[i];
        }
        expected.push_back(sum / static_cast<double>(num_clients));
    }

    for (size_t i = 0; i < averages.size(); i++) {
        //cout << averages[i] << "\t" << expected[i] << "\t" << averages[i] - expected[i] << endl;
        assert(abs(averages[i] - expected[i]) < 1e-8);
    }

    // calc error normal distribution
    double mu = 0.0;
    for (size_t i = 0; i < averages.size(); i++) {
        mu += abs(averages[i] - expected[i]);
    }
    mu /= static_cast<double>(averages.size());

    double sigma = 0.0;
    for (size_t i = 0; i < averages.size(); i++) {
        double x = abs(averages[i] - expected[i]) - mu;
        sigma += x*x;
    }
    sigma /= static_cast<double>(averages.size());
    sigma = sqrt(sigma);

    cout << "Error normal distribution parameters" << endl << " Average: " << mu << endl << " Std. deviation: " << sigma << endl;

    return 0;
}

JNIEXPORT void JNICALL Java_org_apache_sysds_runtime_controlprogram_paramserv_homomorphicEncryption_SEAL_sayHello
